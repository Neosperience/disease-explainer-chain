...
Answer the questions following these instructions: \
<instructions>
...
A: I'm sorry, I can only answer questions about certain diseases. \
Example 3: \
...
Example 6: \
Q: What is HIV? \
A: I apologize, I don't have informations about that. You can ask me about hypertension or diabetes mellitus instead. \
</examples>
---
Context: <context>{context}</examples>
---
...
def load_chain(_prompt_template,_vectorstore):
    ...
    llm = BedrockChat(client=session.client("bedrock-runtime"),model_id="anthropic.claude-3-sonnet-20240229-v1:0",model_kwargs={"temperature": temperature})
...
def main():
    ...
    if "messages_app2" not in st.session_state:
        st.session_state.messages_app2 = []

    for message in st.session_state.messages_app2:
    ...
    if user_input:= st.chat_input("Ask a question"):
        ...
        st.session_state.messages_app2.append({"role": "user", "content": user_input})
        ...
        with st.chat_message('assistant', avatar='https://media.istockphoto.com/id/1364448514/vector/stethoscope-icon-vector-stethoscope-icon-for-medical-design.jpg?s=612x612&w=0&k=20&c=DXnAQgqevXLxd2M650fk5TUligIklWRgsxsJvzm3Hqs='):
            if type(response['source_documents'][0].metadata['source']) is str:
                answer = response['answer'] + "\n\n" + '**Further reading**: ' + response['source_documents'][0].metadata['source']
            else:
                answer = response['answer']
            st.write(answer)
        st.session_state.messages_app2.append({"role": "assistant", "content": answer})
